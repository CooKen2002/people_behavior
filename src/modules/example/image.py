...
if len(indices) > 0:
    for i in indices.flatten():
        ...
        cv2.rectangle(original_image.frame, (left, top), (left + width, top + height), (0, 255, 0), 2)
        
        # Create and draw text label
        label = f"Class {class_id}_{class_name}: {confidence:.2f}"
        cv2.putText(original_image.frame, label, (left, top - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
